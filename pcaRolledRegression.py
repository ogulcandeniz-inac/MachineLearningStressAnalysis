...
# Tahmin değerlerini yuvarla
ypred_pca_rounded = np.round(ypred_pca)

mse_pca = mean_squared_error(ytest_pca, ypred_pca_rounded)
res_pca = 1 - mse_pca
print(f'Mean Squared Error (PCA - Rounded): {mse_pca}')
print("Dogruluk orani (PCA - Rounded): " + str(res_pca))

# Farkları görselleştirme (örneğin, histogram olarak)
differences_pca = ytest_pca - ypred_pca_rounded
plt.hist(differences_pca, bins=20)
plt.xlabel('Gerçek - Tahmin (PCA - Rounded)')
plt.ylabel('Frekans')
plt.title('PCA ile Boyut Azaltılmış Veride Gerçek ve Tahmin Arasındaki Farklar (Yuvarlanmış)')
plt.show()
...
prediction_pca_rounded = np.round(prediction_pca[0][0])
print("Tahmin Edilen Stres Seviyesi (PCA - Rounded):", prediction_pca_rounded)
